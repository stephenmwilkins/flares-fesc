...
for spec_type in ['DustModelI',  'Intrinsic']:
    ...
    ax.scatter(np.log10(D['HbetaEW'][s]), beta[s], s=1, alpha=0.5, c = cm.viridis(norm(np.log10(D['Mstar_30'][s]))))
    ...
    fig.savefig(f'figs/beta_HbEW_{spec_type}.pdf')
    fig.clf()
